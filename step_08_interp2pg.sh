...
if [ -f $infolder"mes_20ensemble_"$year"0801.tif" ]; then
    FILE=$infolder"mes_20ensemble_"$year"0801.tif"
    ...
    r.in.gdal --overwrite -o input=$FILE output=$FILENAME
    ...
    v.out.postgis --overwrite input="chaiken" type="area" output=PG:dbname=obahia output_layer=vector."onset_forecast0108" options="SRID=4326"

    v.db.renamecolumn map="chaiken" column="mae_average,mae_avr"

    v.out.ogr --overwrite -s -e input="chaiken" type="area" output=$outfolder$FILENAME"0108.shp" format="ESRI_Shapefile"
fi

if [ -f $infolder"mes_20ensemble_"$year"0901.tif" ]; then
    FILE=$infolder"mes_20ensemble_"$year"0901.tif"
    FILENAME=$(echo $FILE|sed 's/.*\///')
    FILENAME=$(echo $FILENAME|sed -r 's/.tif//g')
    ...
    db.login --overwrite driver=pg database=obahia user=geonode password=uppQAOFa host=obahia.dea.ufv.br port=5432
    v.out.postgis --overwrite input="chaiken" type="area" output=PG:dbname=obahia output_layer=vector."onset_forecast0109" options="SRID=4326"

    v.db.renamecolumn map="chaiken" column="mae_average,mae_avr"

    v.out.ogr --overwrite -s -e input="chaiken" type="area" output=$outfolder$FILENAME"0109.shp" format="ESRI_Shapefile"
fi
